...
function Import-Card {
    ...
    if ($MLVFSEnabled -and $FailedFiles -eq 0) {
        ...
        if (Test-Path -LiteralPath $ControllerPath) {
            ...
            Write-Log "Invoking batch mount script post-copy: $ControllerPath mount $MLVDestination"

            try {
                & "$ControllerPath" "mount" "$MLVDestination"
                Write-Host "MLVFS mounted successfully to Z:\" -ForegroundColor Green
            }
            catch {
